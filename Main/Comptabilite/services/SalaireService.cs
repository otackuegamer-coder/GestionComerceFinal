...
namespace Superete.Main.Comptabilite.Services
{
    public class SalaireService
    {
        ...
        private SalaireModel MapFromReader(SqlDataReader reader)
        {
            ...
            decimal dec(string col) => reader.IsDBNull(ord(col)) ? 0m : reader.GetDecimal(ord(col));

            return new SalaireModel
            {
                SalaireID = reader.GetInt32(ord("SalaireID")),
                EmployeID = reader.GetInt32(ord("EmployeID")),
                NomComplet = reader.IsDBNull(ord("NomComplet")) ? "" : reader.GetString(ord("NomComplet")),
                CIN = reader.IsDBNull(ord("CIN")) ? null : reader.GetString(ord("CIN")),
                CNSS = reader.IsDBNull(ord("CNSS")) ? null : reader.GetString(ord("CNSS")),
                Mois = reader.GetInt32(ord("Mois")),
                Annee = reader.GetInt32(ord("Annee")),
                DatePaiement = reader.IsDBNull(ord("DatePaiement")) ? (DateTime?)null : reader.GetDateTime(ord("DatePaiement")),
                SalaireBase = dec("SalaireBase"),
                HeuresNormales = dec("HeuresNormales"),
                TauxHoraire = dec("TauxHoraire"),
                PrimeAnciennete = dec("PrimeAnciennete"),
                PrimeRendement = dec("PrimeRendement"),
                PrimeResponsabilite = dec("PrimeResponsabilite"),
                IndemniteTransport = dec("IndemniteTransport"),
                IndemniteLogement = dec("IndemniteLogement"),
                AutresPrimes = dec("AutresPrimes"),
                HeuresSupp25 = dec("HeuresSupp25"),
                HeuresSupp50 = dec("HeuresSupp50"),
                HeuresSupp100 = dec("HeuresSupp100"),
                MontantHeuresSupp = dec("MontantHeuresSupp"),
                SalaireBrut = dec("SalaireBrut"),
                CotisationCNSS = dec("CotisationCNSS"),
                CotisationAMO = dec("CotisationAMO"),
                CotisationCIMR = dec("CotisationCIMR"),
                MontantIR = dec("MontantIR"),
                AvanceSurSalaire = dec("AvanceSurSalaire"),
                PretPersonnel = dec("PretPersonnel"),
                Penalites = dec("Penalites"),
                AutresRetenues = dec("AutresRetenues"),
                TotalRetenues = dec("TotalRetenues"),
                SalaireNet = dec("SalaireNet"),
                CotisationPatronaleCNSS = dec("CotisationPatronaleCNSS"),
                CotisationPatronaleAMO = dec("CotisationPatronaleAMO"),
                Statut = reader.IsDBNull(ord("Statut")) ? "En Attente" : reader.GetString(ord("Statut")),
                Remarques = reader.IsDBNull(ord("Remarques")) ? null : reader.GetString(ord("Remarques")),
                CreePar = reader.IsDBNull(ord("CreePar")) ? null : reader.GetString(ord("CreePar")),
                DateCreation = reader.IsDBNull(ord("DateCreation")) ? (DateTime?)null : reader.GetDateTime(ord("DateCreation")),
                ModifiePar = reader.IsDBNull(ord("ModifiePar")) ? null : reader.GetString(ord("ModifiePar")),
                DateModification = reader.IsDBNull(ord("DateModification")) ? (DateTime?)null : reader.GetDateTime(ord("DateModification"))
            };
        }
    }
}